...
var CNT_THRESHOLD, argv, connect, counter, dgram, error_handler, fs, gauge, getStore, http, http_handlers, initStores, lss, shutdownStores, socket_buffer, socket_io, srv, startHTTP, startSocket, startUDP, stores, udp_handlers, url;
...
startHTTP = function(http_port) {
  ...
  app = connect().use(connect.logger('dev')).use("/js/lib", connect["static"]("" + __dirname)).use(connect["static"]("" + __dirname + "/../public")).use("/components", connect["static"]("" + __dirname + "/../bower_components")).use(connect.query()).use(function(req, res) {
...
srv = startHTTP(argv['http-port'] || 1234);
